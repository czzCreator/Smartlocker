...
class HttpRequestPrivate : public QObject
{
    ...
    Q_PROPERTY(HttpRequestImp::State readyState READ getReadyState WRITE setReadyState NOTIFY readyStateChanged)
    Q_PROPERTY(HttpRequestImp::NetworkStatus status READ getStatus WRITE setStatus NOTIFY statusChanged)
...
public:
    ...
    HttpRequestPrivate(QNetworkAccessManager* networkManager, QObject* parent = Q_NULLPTR):
        ...
        reply(Q_NULLPTR),
        readyState(HttpRequestImp::UnStart),
        status(HttpRequestImp::NoError),
    ...
    HttpRequestImp::State getReadyState() const {
        return readyState;
    }

    void setReadyState(const HttpRequestImp::State &value) {
        if(readyState != value) {
            readyState = value;
    ...
    HttpRequestImp::NetworkStatus getStatus() const {
        return status;
    }

    void setStatus(const HttpRequestImp::NetworkStatus &value)  {
        if(status != value ) {
            status = value;
    ...
    void clear() {
        ...
        this->setReadyState(HttpRequestImp::UnStart);
        this->setMethodName("");
...
private Q_SLOTS:
    void onFinished() {
        if(reply) {
            ...
            this->setStatus((HttpRequestImp::NetworkStatus)e);
            rawHeaderPairs = reply->rawHeaderPairs();

            if(e != QNetworkReply::NoError) {
                this->setReadyState(HttpRequestImp::Error);
                this->setStatusText(reply->errorString());
                Q_EMIT error();
                // error Not finished
                return ;
            } else {
                this->setReadyState(HttpRequestImp::Finished);
                this->setStatusText("");
            }
...
private:
    ...
    HttpRequestImp::State readyState;
    HttpRequestImp::NetworkStatus status;
    QString statusText;
